...
import { lineDistance } from './lib/geo';
import { DAY_MS, dayKey } from './lib/time';
...
export function deriveTimelineStats(
  ...
  const visitSampleKeys = new Set(
    timeline.visits.features.map((visit) =>
      locationKey(
  ...
  for (const route of routes) {
    const routeDistance =
      route.properties.distanceMeters ?? lineDistance(route.geometry.coordinates);
    distanceMeters += routeDistance;
    routePointCount += route.geometry.coordinates.length;
    modeDistances.set(
      route.properties.mode,
      (modeDistances.get(route.properties.mode) ?? 0) + routeDistance,
    );
  ...
  for (const visit of visits) {
    const duration =
      visit.properties.durationMinutes ??
      Math.max(1, (visit.properties.end - visit.properties.start) / 60000);
  ...
  for (const sample of timeline.heatSamples) {
    ...
    const sampleIsVisit = visitSampleKeys.has(
      locationKey(sample.lat, sample.lng, sample.start, sample.end),
    );
  ...
  const trend = trendKeys(timeline.coverage.start, timeline.coverage.end, trendGranularity).map(
    (key) =>
      buckets.get(key) ?? {
        key,
        label: bucketLabel(key, trendGranularity),
  ...
  const totalCountryMinutes = [...countryMap.values()].reduce(
    (total, country) => total + country.minutes,
    0,
  );
  const countryBreakdown = [...countryMap.entries()]
    ...
    .map(([country, values]) => ({
      country,
      ...values,
      share: totalCountryMinutes > 0 ? values.minutes / totalCountryMinutes : 0,
    }));
  const yearBreakdown = [...yearMap.entries()]
    .sort(([yearA], [yearB]) => yearA.localeCompare(yearB))
    .map(([year, values]) => ({
      year,
      activeDays: values.activeDays.size,
      distanceMeters: values.distanceMeters,
      visits: values.visits,
      dwellMinutes: values.dwellMinutes,
      movementPoints: values.movementPoints,
    }));
  ...
  const modeBreakdown = [...modeDistances.entries()]
    ...
    .map(([mode, distance]) => ({
      ...
      share: distanceMeters > 0 ? distance / distanceMeters : 0,
    }));
